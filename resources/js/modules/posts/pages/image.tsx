...
import { CalendarClock, Facebook, ImagePlus, Send, TriangleAlert, Upload, X } from 'lucide-react';
...
export default function ImagePost({ data }: Props) {
    const { accounts } = data;
    const [preview, setPreview] = useState<string | null>(null);
    const fileInputRef = useRef<HTMLInputElement>(null);
    const form = useForm<ImagePostForm>({
        account_ids: [],
        caption: '',
        image: null,
        is_scheduled: false,
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        form.post(route('posts.image.store'), {
            preserveScroll: true,
            forceFormData: true,
            onSuccess: () => {
                form.reset();
                ...
                <div className="p-6 max-w-2xl mx-auto w-full space-y-6">
                    ...
                    <div>
                        ...
                        <p className="text-sm mt-1" style={{ color: 'var(--color-muted)' }}>
                            Publish or schedule a photo to one or more Facebook Pages.
                        </p>
                    </div>
                    ...
                    {accounts.length === 0 ? (
                        ...
                        <div className="rounded-2xl p-6" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
                            <form onSubmit={submit} className="space-y-5">
                                ...
                                <div>
                                    ...
                                    {preview ? (
                                        <div className="relative rounded-lg overflow-hidden" style={{ border: '1px solid var(--color-border)' }}>
                                            <img src={preview} alt="Selected preview" className="w-full max-h-72 object-cover" />
                                            <button
                                                type="button"
                                                onClick={removeFile}
                                                title="Remove image"
                                                className="absolute top-2 right-2 flex items-center justify-center w-8 h-8 rounded-lg"
                                        ...
                                        <button
                                            ...
                                            className="w-full flex flex-col items-center justify-center gap-2 py-10 rounded-lg transition-colors duration-150"
                                            style={{
                                                background: 'var(--color-surface-2)',
                                                border: `1.5px dashed ${form.errors.image ? 'var(--color-danger)' : 'var(--color-border-hover)'}`,
                                            }}
                                        >
                                            <ImagePlus size={24} style={{ color: 'var(--color-muted)' }} />
                                ...
                                <div>
                                    <label htmlFor="caption" className="block text-xs font-medium mb-1.5" style={{ color: 'var(--color-muted)' }}>
                                        Caption (optional)
                                    </label>
                                    <textarea
                                        id="caption"
                                        ...
                                        style={{
                                            background: 'var(--color-surface-2)',
                                            border: `1px solid ${form.errors.caption ? 'var(--color-danger)' : 'var(--color-border)'}`,
                                        ...
                                        onBlur={(e) => {
                                            e.currentTarget.style.borderColor = form.errors.caption ? 'var(--color-danger)' : 'var(--color-border)';
                    ...
                    <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--color-muted)' }}>
                        ...
                        Images are uploaded to storage first, then published to the selected Facebook Pages via the Graph API.
                    </div>
                </div>
            </div>
        </AppLayout>
